...
try:
    ...
    genai.configure(api_key='AIzaSyCCHSXBUnQQsGEuVTIXy7lmw7PaHF_St2o')
...
# Define card styles before layout
...
def calculate_arr_by_channel(df):
    ...
    active_pledges['multiplier'] = active_pledges['frequency'].map({
        ...
        'annually':
        1
    }).fillna(1)  # Default to 1 for unknown frequencies
...
def calculate_chapter_arr(df):
    ...
    active_pledges['multiplier'] = active_pledges['frequency'].map({
        ...
        1,
        'One-Time':
        1  # Include one-time donations as annual
    }).fillna(1)
    ...
    # Group by chapter_type, handle missing chapter types
    chapter_arr = active_pledges.groupby(
        'chapter_type')['annual_value'].sum().reset_index()

    # Fill any missing chapter types with 0
...
app.layout = html.Div(
    [
        # Navigation menu with hamburger icon (header)
        html.Div([
            html.Div(
                [
